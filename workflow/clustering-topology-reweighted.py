...
if "snakemake" in sys.modules:
    ...
    reweighting_method = params["metric"]
...
# 3) Then, add the <your_new_method_name> to the list in Snakemake in a dictionary named `params_clustering_topology_reweighting` at key "metric". For example, if the name is "jaccardindex", params_clustering_topology_reweighting = {"metric": ["bc", "jaccardindex"], "clustering":["infomap"]}
...
src, trg, weight = zip(*[(e.source, e.target, e["weight"]) for e in g.es])
group_ids = clustering_models[clustering_method](src, trg, weight)
