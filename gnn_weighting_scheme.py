...
class CommunityDetectionGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        dot_similarity_matrix = torch.matmul(x, x.t()) # x.t() transposes the node embeddings
...
f=open(f'/N/u/fsfatemi/Quartz/detectability_project/results/gnn_reweighting_scheme/gnn_reweighted_leiden_scores_{N}_nodes_{average_k}_k_{q}_q.txt','w')
...
for mixing_rate in mus:
    ...
    for j in range(6):
        #print(j)
        adj_matrix_data = np.loadtxt(f'Networks/PP N {N} q {q}/k {average_k}/{average_k}_{mixing_rate}_{N}_{j}_adj_matrix.txt')
        ...
        for i in range(num_vertices):
            g.add_node(i)
        edge_list = [(i, k) for i in range(num_vertices) for k in range(i , num_vertices) if adj_matrix[i, k] != 0]
    ...
    for j1 in range(6,20):    
        ...
        adj_matrix_data = np.loadtxt(f'Networks/PP N {N} q {q}/k {average_k}/{average_k}_{mixing_rate}_{N}_{j1}_adj_matrix.txt')
        ...
        for i in range(num_vertices):
            g.add_node(i)
        edge_list = [(i, z) for i in range(num_vertices) for z in range(i, num_vertices) if adj_matrix[i, z] != 0]
        g.add_edges_from(edge_list)
        A = np.array(nx.to_numpy_array(g))
        node_features = LaplacianEigenMap(A, dim=input_dim)
        node_features = torch.from_numpy(node_features).to(torch.float32)
