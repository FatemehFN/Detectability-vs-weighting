...
for q in qs:
    f = open(
        f"/N/u/fsfatemi/Quartz/detectability_project/results/three_gnn_reweighting_scheme/three_gnn_reweighted_LA_scores_{N}_nodes_{average_k}_k_{q}_q.txt",
        "w",
    )
    ...
    for mixing_rate in mus:
        ...
        for j in range(20):
            ...
            # Load data
            A = sparse.load_npz(
                f"Networks/PP N {N} q {q}/k {average_k}/{average_k}_{mixing_rate}_{N}_{j}_adj_matrix.npz"
            )
            ...
            weights = np.sum(emb[src, :] * emb[trg, :], axis=1).reshape(-1)
            g = igraph.Graph.TupleList(
                [[src[i], trg[i], weights[i]] for i in range(len(src))],
                weights=True,
                directed=False,
            )
            # weighted_graph = predict_weights(g, np.abs(emb_to_use))
            part = CLO.leiden_max_modularity_part_weighted(g, trials=100)
